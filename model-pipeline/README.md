...
This directory is the **model-development** layer: translation and evaluation on top of **`data/processed/emotions/<split>/`** (manifests, WAVs, etc.) produced by the data pipeline.
You can override the root with `PIPELINE_DATA_DIR` for other layouts (e.g. `data/processed/stt`).
...
## 1. Overview
 ...
 - The model pipeline runs on top of `data/processed/emotions/<split>/` by default and is **config-driven** (YAML + prompts), not hardcoded.
